...
import { LineChart, Activity, ShieldCheck, ShieldX, ExternalLink } from "lucide-react";
import { apiUrl } from "./apiBase";

function Analytics() {
  const grafanaDashboardUrl = (import.meta.env.VITE_GRAFANA_DASHBOARD_URL || "").trim();
  const useGrafana = Boolean(grafanaDashboardUrl);
  ...
  return (
    <div className="analytics-page dark-theme">
      <div className="analytics-header">
        <div className="header-top">
          <div>
            ...
            <p className="page-subtitle">Built-in analytics (no Docker required)</p>
          </div>
        </div>
      ...
      {loadError ? <div className="graph-error">{loadError}</div> : null}
              ...
              <div className="grid-lines">
                {[0, 25, 50, 75, 100].map((percent, i) => (
                  <div key={i} className="grid-line" style={{ bottom: `${percent}%` }}>
                    <span className="grid-label">{Math.round((percent / 100) * view.maxValue).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="line-graph">
                <div className="line-path total" style={{ clipPath: calculateLinePath(view.totalRequests) }}></div>
        ...
        <div className="graph-card line-graph-card graph-card--success">
          ...
          <div className="graph-content">
            <div className="line-graph-container">
              ...
              <div className="line-graph">
                <div className="line-path success" style={{ clipPath: calculateLinePath(view.successRequests) }}></div>
        ...
        <div className="graph-card line-graph-card analytics-card--wide graph-card--blocked">
          ...
          <div className="graph-content">
            <div className="line-graph-container">
              ...
              <div className="line-graph">
                <div className="line-path blocked" style={{ clipPath: calculateLinePath(view.blockedRequests) }}></div>
